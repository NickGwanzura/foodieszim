      ...
      <thead>
        <tr>
          {columns.map((col) => (
            <th
              key={col.key}
              className="bg-[#f4f4f4] px-4 py-2.5 text-left text-xs font-semibold text-[#525252] uppercase tracking-wide border-b border-[#e0e0e0] whitespace-nowrap"
      ...
      <tbody>
        {data.map((item) => (
          <tr key={keyExtractor(item)} className="transition-colors hover:bg-[#f4f4f4]">
            {columns.map((col) => (
              <td key={col.key} className="px-4 py-3 border-b border-[#e0e0e0] text-sm align-middle">
